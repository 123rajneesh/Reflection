...
namespace EF6CodeFirstReflection
{
    ...
    {
        public static IList<ORMMetadata> mappingList = new List<ORMMetadata>();
        public static void Main(string[] args)
        {
            ...
            if (isIntiger)
            {
                ...
                {
                    case 1:
                        CodeFirstAssemblypath();
                        break;
                    case 2:
                        FluentAPIAssemblypath();
                        GetEntityMappingMetadta();
                        break;
                    default:
        ...
        private static void GetTypesFromAssembly(Type[] typeInAssembly)
        {
            foreach (Type t in typeInAssembly)
            {
                ...
                if (t.BaseType.FullName == "System.Data.Entity.DbContext")
                {
                    ...
                    var mm = t.GetMethods();
                    if (prop[0].PropertyType.Name == "DbSet`1")
                    {
                        var arg = prop[0].PropertyType.GetGenericArguments();
                        string assembly = arg[0].Assembly.GetName().Name;
                        string classNamer = arg[0].Name;

                    }

                    var runtimeMethods = t.GetRuntimeMethods();

                    foreach (var rm in runtimeMethods)
                    {
                        if (rm.Name == "OnModelCreating")
                        {
